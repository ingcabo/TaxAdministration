...
<span style="text-align:left" class="titulo_maestro">
	Generar Archivo TXT de retenciones IVA 
</span>
<center>
	...
	function imprimir()
	{
		...
		var escEnEje = <?=$escEnEje?>;
			if($('lapso').value==''){
				...
				var url = 'json.php';
				var pars = 'op=generaTXT&rango=' + $('lapso').value +'&ms='+ new Date().getTime();
				var Request = new Ajax.Request(
				url,{
				method: 'get',
				parameters: pars,
				...
				onComplete:function(request){
					...
					if(JsonRec){
						Popup= window.open("GenerarArchivosTxtA.php?archivo=IVA.txt","winX","width=500,height=500,scrollbars=yes,resizable=yes,status=yes");
					}else{
