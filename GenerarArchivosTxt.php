...
<form name="form1" action="<? echo $PHP_SELF ?>">
	<table width="400"  >
		<tr>
			...
			<td >
				<select id="Tipo"  name="Tipo">
					...
					<option value="NOMINACENTRAL.txt">Nomina (Banco CENTRAL)</option>
                    <option value="NOMINAVENEZUELA.DAT">Nomina (Banco VENEZUELA)</option>
					...
					<option value="FIDEICOMISO.txt">Fideicomiso BOD</option>
					<option value="NOMINA.xlsx">Excel Nomina</option>
				</select>
			</td>
		</tr>		
		<tr >
		...
		<tr >
			<td >Acumulado:</td>
			<td ><SELECT name="Acumulado" id="Acumulado" ></SELECT></td>
		</tr>
		<tr>
			<td >Banco:</td>
		...
		<tr>
			<td >Concepto Acumulado:</td>
			<td ><?=helpers::combonomina($conn, '', '','','','ConceptoA','int_cod','conc_nom','ConceptoA','','SELECT int_cod,conc_nom FROM rrhh.concepto WHERE conc_tipo=2 ORDER BY int_cod','','true');?></td>
		</tr>
		<tr>
			<td >Periodo:</td>
			<td > 
		...
		<tr>
			<td align="right" colspan="2"><br /><input type="button" value="Generar Archivo" onclick="Generar()" ></td>
		</tr>
	</table>
</form>
</div>
...
var JsonAux;
	...
	//		asynchronous:true, 
			onComplete:function(request){
				var JsonRec = eval( '(' + request.responseText + ')');
				if(JsonRec){
					$('Acumulado').options[0]= new Option("Seleccione",-1);
					for(var i=1;i<=JsonRec.length;i++){
						$('Acumulado').options[i]= new Option(JsonRec[i-1]['D'],JsonRec[i-1]['CI']);
					}
				}
			}
		}
...
var JsonAux;
	...
	}else if($('Nomina').options[$('Nomina').selectedIndex].value== -1 &&( $('Tipo').options[$('Tipo').selectedIndex].value=="NOMINACENTRAL.txt" || $('Tipo').options[$('Tipo').selectedIndex].value=="NOMINAVENEZUELA.DAT")){
	alert("Debe Escojer una Nomina");
		...
		"Acumulado":$('Acumulado').options[$('Acumulado').selectedIndex].value,"ConceptoA":$('ConceptoA').options[$('ConceptoA').selectedIndex].value};
		//alert(JsonAux);
		...
		//		asynchronous:true, 
				onComplete:function(request){
					...
					var JsonRec = eval( '(' + request.responseText + ')');
					if($('Tipo').options[$('Tipo').selectedIndex].value!="NOMINA.xlsx"){
						if(JsonRec){
						...
						if(JsonRec){
							Popup= window.open("GenerarArchivosExel.php?archivo="+$('Tipo').options[$('Tipo').selectedIndex].value,"winX","width=500,height=500,scrollbars=yes,resizable=yes,status=yes");
						}else{
							alert("No se pudo generar el archivo");
						}
					}
