...
class documentos_generales{
	...
	*****************************/
	...
	function get($conn, $id, $escEnEje=''){
		$q = "SELECT documentos_generales.*, unidades_ejecutoras.descripcion AS unidad_ejecutora, ";
		...
		$q.= "FROM puser.documentos_generales ";
		$q.= "INNER JOIN puser.unidades_ejecutoras ON (documentos_generales.id_unidad_ejecutora = unidades_ejecutoras.id) ";
		$q.= "INNER JOIN puser.proveedores ON (documentos_generales.id_proveedor = proveedores.id) ";
		$q.= "WHERE documentos_generales.id='$id'  ";
		//die($q);
		$r = $conn->Execute($q);
		if(!$r->EOF){
			$this->id = $r->fields['id'];
	...
	function get_all($conn, $escEnEje,$orden="id"){
		$q = "SELECT * FROM puser.documentos_generales ";
		$q.= "ORDER BY $orden ";
		$r = $conn->Execute($q);
		$collection=array();
		while(!$r->EOF){
			$ue = new documentos_generales;
	...
	function add($conn, 
		...
		$q = "INSERT INTO puser.documentos_generales ";
		$q.= "( ";
		$q.= "id_tipo_documento, ";
		...
		$r = $conn->execute($q);
		if($r){
				$nrodoc = getLastId($conn, 'id', 'documentos_generales'); 
				if(
	...
	function set($conn, 
		...
		$q = "UPDATE puser.documentos_generales SET  ";
	...
	function del($conn, $id){
		$q = "DELETE FROM puser.documentos_generales WHERE id='$id'";
		if($conn->Execute($q)){
			$this->msj = REG_DEL_OK;
			return true;
		}else{
	...
	function addRelacionPartidas($conn, 
		...
		if(is_array($JsonRec->servicio)){
			foreach ($JsonRec->servicio as $oCS_Aux){
				$CS_Aux_2= $oCS_Aux[2];
			$q = "INSERT INTO puser.relacion_doc_generales ";
			$q.= "( id_parcat, id_categoria_programatica, id_partida_presupuestaria, monto, id_doc_generales,porc_iva,monto_exc) ";
			$q.= "VALUES ";
			$q.= "('$oCS_Aux[7]', '$oCS_Aux[0]', '$oCS_Aux[1]', '$CS_Aux_2', '$nrodoc', '$oCS_Aux[4]', '$oCS_Aux[3]') ";
			//die($q);
			//echo($q."<br>");
			$r = $conn->Execute($q);
		} 
	...
	function delRelacionPartidas($conn, $nrodoc){
		$q = "DELETE FROM puser.relacion_doc_generales WHERE id_doc_generales='$nrodoc'";
		if($conn->Execute($q))
			return true;
		else
			return false;
	...
	function getRelacionPartidas($conn, $id, $escEnEjec){
		$q = "SELECT relacion_doc_generales.*, partidas_presupuestarias.descripcion AS partida_presupuestaria,  ";
		$q.= "categorias_programaticas.descripcion AS categoria_programatica ";
		$q.= "FROM puser.relacion_doc_generales  ";
		$q.= "INNER JOIN puser.partidas_presupuestarias ON (relacion_doc_generales.id_partida_presupuestaria = partidas_presupuestarias.id) ";
		$q.= "INNER JOIN puser.categorias_programaticas ON (relacion_doc_generales.id_categoria_programatica = categorias_programaticas.id) ";
		$q.= "WHERE relacion_doc_generales.id_doc_generales='$id' ";
	...
	function aprobar($conn, $id_doc_generales,
										$id_usuario,
										$id_unidad_ejecutora,
										$id_proveedor,
										$ano,
		...
		for($i = 0; $i < $contador; $i++){
			...
			$r = $conn->Execute($q);
			if($r){
				if($r->fields['disponible'] < $JsonRec->servicio[$i][2]){
					$this->msj = ERROR_DG_APR_NO_DISP;
					return false;
				}
			}
		} 
		...
		foreach($JsonRec->servicio as $servicio){
			...
			$aMonto[] 					= muestrafloat($servicio[2]);
		...
		if($r){
			...
			$c = $this->setNrodoc($conn, $nrodoc, $id_doc_generales);
			
			#ACTUALIZO LA LA FECHA DE APROBACION Y EL ESTATUS DE LA ORDEN DE PAGO#
			$d = $this->setFechaAprobacion($conn, $id_doc_generales);
	...
	function setNrodoc($conn, $nrodoc, $id){
		$q = "UPDATE puser.documentos_generales SET nrodoc = '$nrodoc' WHERE id='$id'";
		//die($q);
		if($conn->Execute($q))
			return true;
		else
	...
	function setFechaAprobacion($conn, $id){
		$q = "UPDATE puser.documentos_generales SET fecha_aprobacion = now(), status = '2' WHERE id='$id'";
		//die($q);
		if($conn->Execute($q))
			return true;
		else
	...
	#PARA ANULAR EL CONTRATO DE SERVICIO
	...
	function anular($conn, $id, $id_usuario,
		...
		try{
		$q3 ="UPDATE puser.documentos_generales SET status=3 WHERE id=$id";
