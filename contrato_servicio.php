...
if($accion == 'Guardar'){
	...
	$oContratoServicio->aprobar($conn, 
									$_POST['id'], 
									$usuario->id,
									$_POST['unidad_ejecutora'],
									$_POST['proveedores'],
	...
	$oContratoServicio->del($conn, $_POST['id']);
}elseif($accion == 'Anular'){
	$oContratoServicio->anular($conn, $_POST['id'],
									  $usuario->id,
									  $_POST['unidad_ejecutora'],
									  date("Y"),
									  $_POST['descripcion'],
...
$msj = $oContratoServicio->msj; // lleno esta variable con el mensaje de la operacion llevada a cabo
...
<br />
<span class="titulo_maestro">Contrato de Servicio</span>
<div id="formulario">
...
function busca(id_ue, id_proveedor, descripcion, fecha_desde, fecha_hasta, nrodoc, pagina){
	var url = 'updater_busca_contrato_servicio.php';
...
//Nueva revision************ 27/09/2006
	...
	var pars = 'filtro=401,403&cp=' + cp +'&ms='+new Date().getTime();
...
{
	var url = 'buscar_partidas.php';
	var pars = 'filtro=401,403&cp=' + $('cp').value +'&nombre='+$('search_nombre_pp').value+'&codigo='+$('search_cod_pp').value+'&opcion=2&ms='+new Date().getTime();
...
function buscaNroDoc(boton){
	...
	function calculaImp(){
		...
		var montoImp = usaFloat($('montoCS').value) * (valor/100);
		alert(montoImp);	
	...
	function generaTotal(rowId){
		if (mygridcs.cells(rowId,4).getValue()==''){
			...
			return false;
		} else if (parseFloat(mygridcs.cells(rowId,3).getValue()) > usaFloat(mygridcs.cells(rowId,2).getValue())){
	...
	function muestraCategoriaProgramatica(ue){
	var url = 'json.php';
	var pars = 'op=muestracategoria&ue=' + ue;
	...
	var myAjax = new Ajax.Request(
				...
				onComplete: function(peticion){
					var jsonData = eval('(' + peticion.responseText + ')');
					if (jsonData == undefined) { return }
					//alert(jsonData.descripcion);
					$('txtcategorias_programaticas').value = jsonData[0].descripcion;
					$('categorias_programaticas').value = jsonData[0].id_categoria;
				}
				}
			);
}
...
$validator->create_message("error_id_servicio", "id_servicio", "*");
