...
if($accion == 'Guardar'){
	...
	$oCheque->Anular($conn,$_REQUEST['nrodoc'],$escEnEje,$_REQUEST['status'],$_REQUEST['observacion'],$usuario->id,$anoCurso,$_REQUEST['nomBenef'],$_REQUEST['concepto'],guardafecha($_REQUEST['fecha']));
}
...
<fieldset id="buscador">
	...
	<table border="0">
		...
		<tr>
			...
			<td><input style="width:100px" type="text" name="busca_nrocheque" id="busca_nrocheque" /></td>
		</tr>
		<tr>
...
function traeCuentasBancarias(id_banco, div, id_cuenta,disabled){
	...
	var pars = 'combo=cuentas_bancarias3&id_banco=' + id_banco + '&id_cuenta=' + id_cuenta + '&disabled=' + disabled +'&ms='+new Date().getTime();
...
function traeCuentasBancarias2(id_banco, div, id_cuenta){
	var url = 'updater_selects.php';
	var pars = 'combo=cuentas_bancarias3&id_banco=' + id_banco + '&id_cuenta=' + id_cuenta +'&ms='+new Date().getTime();
	var updater = new Ajax.Updater(div, 
		url,
		{
			method: 'get',
...
function traeBancos(id_banco, div){

	var url = 'updater_selects.php';
	var pars = 'combo=banco&id_banco=' + id_banco + '&ms='+new Date().getTime();
	var updater = new Ajax.Updater(div, 
		url,
		{
			method: 'get',
...
function busca(id_banco, id_cuenta, id_proveedor, fecha_desde, fecha_hasta, nrodoc, nrocheque, pagina){
	var url = 'updater_busca_cheque.php';
	var pars = '&id_banco=' + id_banco + '&id_cuenta=' + id_cuenta + '&id_proveedor=' + id_proveedor;
	pars += '&nrodoc=' + nrodoc + '&nrocheque=' + nrocheque + '&fecha_desde=' + fecha_desde + '&fecha_hasta=' + fecha_hasta + '&pagina=' + pagina;
	//alert(pars);
...
Event.observe('busca_nrocheque', "change", function () { 
	busca($F('busca_bancos'),
	$F('busca_nro_cuenta'), 
	$F('busca_proveedores'),
	$F('busca_fecha_desde'), 
