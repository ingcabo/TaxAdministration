...
$escenario = $_REQUEST['id_escenario']; 	
		...
		$rppcp = $oCp->getProyecto($conn, $escenario);
		//var_dump($rppcp);
...
class PDF extends FPDF
{
  ...
  var $escEnEje;
	...
	function Header()
	{
			...
			$this->MultiCell(0,2, "DESCRIPCION DEL PROYECTO", 0, 'C');
			...
			$this->Line(14.5, 210, 14.5, 80);
			$this->Line(195, 210, 195, 80);
...
if (is_array($rppcp)){
	foreach($rppcp as $cp){
		...
		$pdf->Ln();
		$pdf->SetFont($pdf->fontStyle, '', $pdf->fontBodySize);
		$pdf->Cell(($pdf->cellColWidth*5)+1, 4, 'Programa', LRB, '', 'L');
		$pdf->SetFont($pdf->fontStyle, '', $pdf->fontBodySize);
		$pdf->Cell($pdf->cellColWidth, 4, substr($cp->id,2,2), RB, '', 'C');
		$pdf->Cell($pdf->cellDescWidth+$pdf->cellTotalWidth, 4, utf8_decode($cp->programa), B, '', 'C');
		$pdf->Ln();
		$pdf->SetFont($pdf->fontStyle, '', $pdf->fontBodySize);
		$pdf->Cell(($pdf->cellColWidth*5)+1, 4, 'Proyecto', LRB, '', 'L');
		$pdf->SetFont($pdf->fontStyle, '', $pdf->fontBodySize);
		$pdf->Cell($pdf->cellColWidth, 4, substr($cp->id,8,2), RB, '', 'C');
		$pdf->Cell($pdf->cellDescWidth+$pdf->cellTotalWidth, 4, utf8_decode($cp->descripcion), BR, '', 'C');
		$pdf->Ln();
		$pdf->SetFont($pdf->fontStyle, '', $pdf->fontBodySize);
		$pdf->Cell(($pdf->cellColWidth*6)+1, 4, 'Unidad Ejecutora', LR, '', 'L');
		$pdf->SetFont($pdf->fontStyle, '', $pdf->fontBodySize);
		$pdf->Cell($pdf->cellDescWidth+$pdf->cellTotalWidth, 4, utf8_decode($cp->ue), R, '', 'C');
		$pdf->Ln();
