...
$oDocumentosGenerales = new documentos_generales;
$mPresupuestario = new movimientos_presupuestarios;
$oDocumentosGenerales->get($conn, $_GET['id'],$escEnEje);
if(empty($oDocumentosGenerales->nrodoc))
	header ("location: contrato_servicio.php");
$_SESSION['pdf'] = serialize($oDocumentosGenerales);
...
//Cabecera de página
	function Header()
	{
			...
			$oDocumentosGenerales = unserialize($_SESSION['pdf']);

			$this->SetXY(150, 20); 
			$textoDerecha = "Fecha: ".date('d/m/Y')."\n";
			$textoDerecha.= "Fecha Generac.:".muestrafecha($oDocumentosGenerales->fecha)."\n";
			$textoDerecha.= "Fecha Aprob.:".muestrafecha($oDocumentosGenerales->fecha_aprobacion)."\n";
			$textoDerecha.= "Pag: ".$this->PageNo()." de {nb}\n";
			$this->MultiCell(50,2, $textoDerecha, 0, 'L');
			
			$this->SetFont('Courier','b',12);
			$this->Text(80, 40, "Documentos Generales");
			$this->Text(153, 40, "Nro.:".$oDocumentosGenerales->nrodoc."\n");
...
$oProveedor->get($conn, $oDocumentosGenerales->id_proveedor);
//$pdf->SetFillColor(232 , 232, 232);
$pdf->Cell(25,4, 'Ciudadano:',0, '','L' );
$pdf->SetFont('Courier','',8);
$pdf->Cell(100,4, $oDocumentosGenerales->nombre_proveedor,0, '','L');

$pdf->Ln();
$pdf->SetFont('Courier','B',8);
$pdf->Cell(7,4, 'Rif:',0, '','L' );
$pdf->SetFont('Courier','',8);
$pdf->Cell(25,4, $oDocumentosGenerales->rif,0, '','L' );
$pdf->SetFont('Courier','B',8);
$pdf->Cell(7,4, 'Nit:',0, '','L' );
$pdf->SetFont('Courier','',8);
$pdf->Cell(25,4, $oDocumentosGenerales->nit,0, '','L' );
$pdf->SetFont('Courier','B',8);
$pdf->Cell(7,4, 'Fax:',0, '','L' );
$pdf->SetFont('Courier','',8);
$pdf->Cell(25,4, $oDocumentosGenerales->fax,0, '','L' );
$pdf->SetFont('Courier','B',8);
$pdf->Cell(7,4, 'Tlf:',0, '','L' );
$pdf->SetFont('Courier','',8);
$pdf->Cell(25,4, $oDocumentosGenerales->telefono,0, '','L' );

$pdf->Ln();
$pdf->SetFont('Courier','B',8);
$pdf->Cell(25,4, 'Direccion',0, '','L');
$pdf->SetFont('Courier','',8);
$pdf->MultiCell(100,4, $oDocumentosGenerales->dir_proveedor,0, '','L');
...
$pdf->SetFont('Courier','',8);
$pdf->MultiCell(150,4, $oDocumentosGenerales->descripcion,0, '','L');
...
$pdf->SetFont('Courier','',8);
$pdf->Cell(90,4, utf8_decode($oDocumentosGenerales->unidad_ejecutora),0, '','L');
$pdf->Ln();
...
$cPartidas = $mPresupuestario->getImputacionReportes($conn,$oDocumentosGenerales->nrodoc,$escEnEje);
