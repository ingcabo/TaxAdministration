...
class clasificacion_espectaculo{
	...
	function get($conn, $id){
		$q = "SELECT * FROM publicidad.clasificacion_eventos ";
	...
	function get_all($conn, $orden="cod_lugar_evento"){
		$q = "SELECT * FROM publicidad.clasificacion_eventos ";//die($q);
		$q.= "ORDER BY $orden "; 
		$r = $conn->Execute($q);
		$collection=array();
		while(!$r->EOF){
	...
	function add($conn, $descripcion, $status){
		$q = "INSERT INTO publicidad.clasificacion_eventos ";
	...
	function set($conn, $id, $descripcion, $status){
		$q = "UPDATE publicidad.clasificacion_eventos SET descripcion='$descripcion', status=$status ";
		$q.= "WHERE cod_lugar_evento=$id";	
		//die($q);
		if($conn->Execute($q))
			return true;
	...
	function del($conn, $id){
		$q = "DELETE FROM publicidad.clasificacion_eventos WHERE cod_lugar_evento='$id'";
		if($conn->Execute($q))
			return true;
		else
			return false;
