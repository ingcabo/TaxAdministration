...
//Cabecera de página
	function Header()
	{
			...
			$this->Cell(20,0,"Cedula",0, 0,'R');	
			$this->Cell(40,0,"Aporte",0, 0,'R');	
			$this->Cell(50,0,"Retencion",0, 0,'R');	
...
$q = "SELECT DISTINCT A.tra_cod,A.tra_nom,B.tra_ced  FROM rrhh.hist_nom_tra_conc AS A INNER JOIN rrhh.trabajador AS B ON A.tra_cod=B.int_cod WHERE A.hnom_cod=".$_GET['id']." AND (A.tra_cod=".$_GET['Tra']." OR ".$_GET['Tra']."=-1 ) AND B.tra_vac=0 ORDER BY A.tra_nom";
$rT = $conn->Execute($q);
$TotalNomina=0;
$TotalNominaAporte=0;
$Contador=0;
while(!$rT->EOF){
	...
	while(!$rC->EOF) {
		...
		$pdf->Cell(20,0, $rT->fields['tra_ced'],0, 0,'R');	
		$pdf->Cell(40,0,  muestrafloat($rC->fields['aporte']),0, 0,'R');	
		$pdf->Cell(50,0,  muestrafloat($rC->fields['valor']),0, 0,'R');	
