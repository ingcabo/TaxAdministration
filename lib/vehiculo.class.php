...
class vehiculo{
	...
	function get($conn, $id){
		...
		if(!$r->EOF){
			...
			//consulta para la fecha de reincorporacion	
			$w = "SELECT * FROM vehiculo.veh_desincorporado ";
			$w.= "WHERE cod_vehiculo='$id'";
			//die($q);
			$p = $conn->Execute($w);
				if(!$p->EOF){
			$this->fecha = $p->fields['fecha_desincorporacion'];
			
			//consulta para la fecha de reincorporacion			
			$s = "SELECT * FROM vehiculo.veh_reincorporado ";
			$s.= "WHERE cod_vehiculo='$id'";
			//die($q);
			$k = $conn->Execute($s);
				if(!$k->EOF){
			$this->fecha_re = $k->fields['fecha_reincorporacion'];
	...
	function add($conn, $id_contribuyente, $serial_carroceria, $placa, $fec_crea, $anio_veh, $serial_motor, $cod_mar, $cod_mod, $cod_col, $cod_uso,
					$cod_tip, $fec_compra, $peso_kg, $cant_eje, $precio, $observacion, $exento, $anio_pago, $cod_lin, $concesionario, $primera_vez){
		$q = "INSERT INTO vehiculo.vehiculo ";
		$q.= "(id_contribuyente, serial_carroceria, placa, fec_crea, anio_veh, serial_motor, cod_mar, cod_mod, cod_col, cod_uso, cod_tip, 
				fec_compra, peso_kg, cant_eje, precio, observacion, exento, anio_pago, cod_lin, concesionario, primera_vez) ";
		$q.= "VALUES ";
		$q.= "($id_contribuyente, '$serial_carroceria', '$placa', '$fec_crea', $anio_veh, '$serial_motor', $cod_mar, $cod_mod, $cod_col, $cod_uso, $cod_tip, 
				'$fec_compra', '$peso_kg', $cant_eje, '$precio', '$observacion', '$exento', $anio_pago, $cod_lin, $concesionario, $primera_vez) ";
