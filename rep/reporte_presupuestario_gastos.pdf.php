...
$partOrdenadas = array();	
	...
	function buscaMadres($conn, $idPartida, $monto){
...
class PDF extends FPDF
{
  ...
  var $escEnEje;
	...
	function Header()
	{
			...
			$textoCabecera.= ENTE."\n";
			$textoCabecera.= "DIRECCION DE PLANIFICACION Y PRESUPUESTO";
			...
			$this->MultiCell(0,2, "PRESUPUESTO DE GASTOS", 0, 'C');
			...
			$this->Ln();
			$this->SetFont($this->fontStyle, 'B', $this->fontBodySize);
			$this->Cell(($this->cellColWidth*5)+1, 4, 'Sector', 1, '', 'L');
			$this->SetFont($this->fontStyle, '', $this->fontBodySize);
			$this->Cell($this->cellColWidth, 4, $this->codSector, R, '', 'C');
			$this->Cell($this->cellDescWidth+$this->cellTotalWidth, 4, utf8_decode($this->descSector), RB, '', 'C');
			$this->Ln();
			$this->SetFont($this->fontStyle, 'B', $this->fontBodySize);
			$this->Cell(($this->cellColWidth*5)+1, 4, 'Programa', 1, '', 'L');
			$this->SetFont($this->fontStyle, '', $this->fontBodySize);
			$this->Cell($this->cellColWidth, 4, $this->codPrograma, RB, '', 'C');
			$this->Cell($this->cellDescWidth+$this->cellTotalWidth, 4, utf8_decode($this->descPrograma), RB, '', 'C');
			$this->Ln();
			$this->SetFont($this->fontStyle, 'B', $this->fontBodySize);
			$this->Cell(($this->cellColWidth*5)+1, 4, 'Actividad', 1, '', 'L');
			$this->SetFont($this->fontStyle, '', $this->fontBodySize);
			$this->Cell($this->cellColWidth, 4, $this->codActividad, RB, '', 'C');
			$this->Cell($this->cellDescWidth+$this->cellTotalWidth, 4, utf8_decode($this->descActividad), RB, '', 'C');
			$this->Ln();
			$this->SetFont($this->fontStyle, 'B', $this->fontBodySize);
			$this->Cell(($this->cellColWidth*6)+1, 4, 'Unidad Ejecutora', 1, '', 'L');
			$this->SetFont($this->fontStyle, '', $this->fontBodySize);
			$this->Cell($this->cellDescWidth+$this->cellTotalWidth, 4, utf8_decode($this->descUnidad), RB, '', 'C');
			$this->Ln(6);			
...
$pdf=new PDF();
$pdf->codSector = substr($rppcp[0]->id_cp, 0, 2);
$pdf->descSector = $categoria_sector[0]->descripcion;
$pdf->codPrograma = substr($rppcp[0]->id_cp, 2, 2);
$pdf->descPrograma = $categoria_programa[0]->descripcion;
$pdf->codActividad = substr($rppcp[0]->id_cp, 8, 9);
$pdf->descActividad = $rppcp[0]->desc_cp;
$pdf->descUnidad =  $categoria_unidad[0]->descripcion;
