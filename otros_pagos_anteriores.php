...
$oOtrosPagos = new otros_pagos_anteriores;
$accion = $_REQUEST['accion'];
$hoy = date('Y-m-d');	
#ACCION DE GUARDAR EL CHEQUE#
if($accion == 'Guardar'){
	$oOtrosPagos ->add($conn,$_REQUEST['banco'],$_REQUEST['nro_cuenta'],$_REQUEST['nroOtrosPagos'],$_REQUEST['id_proveedor'],guardafecha($_REQUEST['fecha']),0,$_REQUEST['contenedor_ordenes'],$usuario->id,$escEnEje,$anoCurso,$_REQUEST['observacion'],$_REQUEST['total_cheque'],$_REQUEST['descripcion'],$_REQUEST['nro_control']);
...
<fieldset id="buscador">
	...
	<table border="0">
		...
		<tr>
			...
			"SELECT id, nombre AS descripcion FROM proveedores")?></td>
			<td>&nbsp;</td>
		</tr>
		<tr>
			<td >Desde</td>
...
<?
	$validator->create_message("error_observacion", "observacion", "No puede estar vacio");
	$validator->create_message("error_nroOtrosPagos", "nroOtrosPagos", "No puede estar vacio");
...
function busca(id_banco, id_cuenta, id_proveedor, fecha_desde, fecha_hasta, nrodoc, pagina){
	var url = 'updater_busca_otros_pagos_anteriores.php';
