...
//Cabecera de página
	function Header()
	{
			...
			$this->Cell(25,0,"Cedula",0, 0,'R');
			$this->Cell(15,0,"Sexo",0, 0,'C');
			$this->Cell(20,0,"Fecha N.",0, 0,'R');	
...
$q = "SELECT DISTINCT A.tra_cod, A.tra_nom, B.tra_nac, B.tra_ced, B.tra_sex, B.tra_fec_nac FROM rrhh.hist_nom_tra_conc AS A INNER JOIN rrhh.trabajador AS B ON A.tra_cod=B.int_cod WHERE A.hnom_cod=".$_GET['id']." AND (A.tra_cod=".$_GET['Tra']." OR ".$_GET['Tra']."=-1 ) AND B.tra_vac=0 ORDER BY A.tra_nom";
$rT = $conn->Execute($q);
$TotalNomina=0;
$TotalNominaAporte=0;
...
while(!$rT->EOF){
	...
	while(!$rC->EOF) {
		...
		$pdf->Cell(25,0, $rT->fields['tra_ced'],0, 0,'R');
		$pdf->Cell(15,0, $rT->fields['tra_sex']==0 ? 'M':'F',0, 0,'C');
		$pdf->Cell(20,0, muestrafecha($rT->fields['tra_fec_nac']),0, 0,'R');	
