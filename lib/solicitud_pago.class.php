...
class solicitud_pago{
	...
	*****************************/
	...
	function getRelacionPartidas($conn, $id, $escEnEjec){
		...
		$q.= "ORDER BY relacion_solicitud_pago.id ";
		//die($q);
		$r = $conn->Execute($q);
		while(!$r->EOF){
			$ue = new solicitud_pago;
	...
	function get_suma_aportes_sp($conn, $nrodocref){
	    ...
	    $q = "SELECT distinct a.id_partida_presupuestaria ,  SUM(a.monto) as monto, a.id_categoria_programatica AS categoria, a.id_partida_presupuestaria AS partida
			  FROM puser.relacion_movimientos as a 
			  INNER JOIN rrhh.conc_part as cp ON ( a.id_partida_presupuestaria = cp.par_cod and a.id_categoria_programatica = cp.cat_cod )
			  INNER JOIN rrhh.concepto as c ON (c.int_cod = cp.conc_cod)
			  WHERE a.nrodoc = '$nrodocref' AND COALESCE(LENGTH(c.conc_aporte), 0) > 0 
			  GROUP BY a.id_categoria_programatica, a.id_partida_presupuestaria, c.conc_cod, a.monto ORDER BY a.id_categoria_programatica";
	...
	function add_facturas($conn,$nrodoc,$aFacturas){
		...
		if(is_array($JsonRec->facturas)){
			foreach($JsonRec->facturas as $facturas){
				...
				$fecha2 = guardafecha($facturas[2]);
				$q = "INSERT INTO finanzas.facturas_solicitud_pago (
						nrodoc, 
						nrofactura,
						nrocontrol, 
						fecha, 
	...
	function delfacturas($conn, $nrodoc){
		...
		$q = "DELETE FROM finanzas.facturas_solicitud_pago WHERE nrodoc='$nrodoc'";
		//echo "<br>".$q;
		$r = $conn->Execute($q);
	...
	function getfacturas($conn, $nrodoc){
		...
		if ($nrodoc==""){
			...
			$q = "SELECT * FROM finanzas.facturas_solicitud_pago WHERE nrodoc = '$nrodoc'";
			//die($q);
			$r = $conn->Execute($q);
		...
		function getfacturas2($conn, $nrodoc){
		...
		if ($nrodoc==""){
			...
			/*$relacionFacturas = new Services_JSON();
			$relacionFacturas = is_array($coleccion) ? $relacionFacturas->encode($coleccion) : false;*/
			return $coleccion;
		}
			
	}
	//Obtiene las facturas cargadas en caja chica
	function getfacturasCajaChica($conn, $nrodoc){
		...
		if ($nrodoc==""){
			...
			$sql = "SELECT id FROM puser.caja_chica WHERE nrodoc = '$nrodoc'";
			$row = $conn->Execute($sql);
			$q = "SELECT * FROM puser.relacion_factura_caja_chica WHERE id_caja_chica = '".$row->fields['id']."'";
			//die($q);
			$r = $conn->Execute($q);
