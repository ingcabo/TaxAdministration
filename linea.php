...
$status=0;
	...
	if(!empty($_REQUEST['accion']) and $_REQUEST['accion']=='Guardar'){
		$sql="INSERT INTO vehiculo.linea (descripcion, status) VALUES ('$descripcion', $status)";
	...
	if(!empty($_REQUEST['accion']) and $_REQUEST['accion']=='Actualizar'){
		...
		$sql="UPDATE vehiculo.linea SET descripcion='$descripcion', status=".$status." WHERE cod_lin=".$cod_lin;
		$msj="Registro Actualizado con éxito";
	}
		
		$conn->Execute($sql);

$sql="SELECT * FROM vehiculo.linea ORDER BY descripcion ASC";
$rs = $conn->Execute($sql);
print $conn->ErrorMsg();
...
<br />
<span class="titulo_maestro">Maestro de L&iacute;neas de Transporte</span>
<?=$div?>
<br />
...
?>
<tr class="filas"> 
<td><?=$rs->fields['cod_lin']?></td>
<td><?=$rs->fields['descripcion']?></td>
<td align="center"><?=$text_status?></td>
<td align="center"><a href="#" onclick="gettpl(<?=$rs->fields['cod_lin']?>);"><img src="images/actualizar.gif" width="16" height="10" border="0"></a></td>
</tr>
<?php 
			$rs->MoveNext();
		}
