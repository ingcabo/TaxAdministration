...
class operaciones{
	...
	function get($conn, $id){
		...
		$q.="FROM puser.operaciones AS A ";
		$q.="LEFT JOIN puser.operaciones AS B ON A.id_padre=B.id ";
		$q.="LEFT JOIN puser.operaciones AS C ON B.id_padre=C.id ";
		$q.="WHERE A.id='$id'";
		//echo $q."<br>";
		$r = $conn->Execute($q);
		if(!$r->EOF){
...
//		$q.= "AND tipo = 'V' ";
	...
	function add($conn, $descripcion, $pagina, $id_modulo, $id_padre, $tipo, $nivel1, $orden){
		...
		$q.= "VALUES ";
		$q.= "('$descripcion', '$pagina', '$id_modulo', '$id_padre', '$tipo', $nivel1, '$orden' ) ";
		//die($q);
		if($conn->Execute($q))
			return true;
		else
	...
	function set($conn, $id, $descripcion, $pagina, $id_modulo, $id_padre, $tipo, $nivel1, $orden){
		...
		$q.= "id_padre ='$id_padre', tipo = '$tipo', nivel = $nivel1, orden = '$orden' ";
		$q.= "WHERE id='$id' ";
		//die($q);
		if($conn->Execute($q))
			return true;
