...
class inscripcion_publicidad{
	...
	********************************/
	...
	function get($conn, $id){
		$q = "SELECT * FROM publicidad.inscripcion_publicidad ";
		$q.= "WHERE id='$id'";
 		//die($q);
		$r = $conn->Execute($q);
		if(!$r->EOF){
			$this->id = $r->fields['id_inscripcion_publicidad'];  
			$this->fecha = $r->fields['fecha_inscripcion']; 
			$this->patente = $r->fields['patente']; //---
			$this->id_contribuyente = $r->fields['cod_id_contribuyente'];
			$this->cod_ins = $r->fields['cod_inspector'];
			$this->cod_publicidad = $r->fields['cod_tipo_publicidad'];
	...
	function get_all($conn, $from=0, $max=0,$orden="id_inscripcion_publicidad"){
		$q = "SELECT * FROM publicidad.inscripcion_publicidad ";
		$q.= "ORDER BY $orden ";
		//die($q);
		...
		while(!$r->EOF){
			...
			$ue->get($conn, $r->fields['id_inscripcion_publicidad']);
			$coleccion[] = $ue;
			$r->movenext();
		}
	...
	function add($conn, 						
		...
		$q = "INSERT INTO publicidad.inscripcion_publicidad(";
		$q.= "		    fecha_inscripcion,
						patente,
						cod_id_contribuyente,						
						cod_tipo_solicitud,
						cod_inspector,
						cod_tipo_publicidad
						)";
		...
		$w = "INSERT INTO publicidad.resultado_inspeccion(";
		$w.= " 			cod_inspector,
						patente,
						status
						)";
		...
		if ( ($conn->Execute($q)) && ($conn->Execute($w)) )
		{
			return true;
		}
		else
