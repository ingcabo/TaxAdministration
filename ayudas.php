...
if($accion == 'Guardar'){
	...
	$oAyudas->del($conn, $_POST['id']);
}elseif($accion == 'Anular'){
	$oAyudas->anular($conn, $_POST['id'],
									  $usuario->id,
									  $_POST['unidad_ejecutora'],
									  date("Y"),
									  $_POST['descripcion'],
...
$msj = $oAyudas->msj; // lleno esta variable con el mensaje de la operacion llevada a cabo
...
<br />
<span class="titulo_maestro">Ayudas</span>
<div id="formulario">
...
<fieldset id="buscador">
	...
	<table>
		...
		<tr>
			...
			"SELECT id, rif||' - '||nombre AS descripcion FROM proveedores WHERE id <> 54")?></td>
...
function busca(id_ue, id_proveedor, descripcion, fecha_desde, fecha_hasta, nrodoc, pagina){
	var url = 'updater_busca_ayudas.php';
...
//Nueva revision************ 27/09/2006
	...
	var pars = 'filtro=401,403,407&cp=' + cp +'&ms='+new Date().getTime();
...
{
	var url = 'buscar_partidas.php';
	var pars = 'filtro=401,407&cp=' + $('cp').value +'&nombre='+$('search_nombre_pp').value+'&codigo='+$('search_cod_pp').value+'&opcion=2&ms='+new Date().getTime();
	...
	if ($('unidad_ejecutora').value =="0"){
		...
		mygridcs.addRow($('idParCat').value,$('categorias_programaticas').value+";"+$('partidas_presupuestarias').value+";"+$('montoip').value+";0;0;0;0");
		i++;
		sumaTotal();
		$('montoip').value='0,00';
	}
...
function buscaNroDoc(boton){
	...
	function generaTotal(rowId){
		if (mygridcs.cells(rowId,4).getValue()==''){
			...
			var costo = parseFloat(usaFloat(mygridcs.cells(rowId,2).getValue()));
			...
			mygridcs.cells(rowId,5).setValue(muestraFloat(impuesto.toFixed(2)));		
			mygridcs.cells(rowId,6).setValue(muestraFloat(total.toFixed(2)));
			sumaTotal();
	   }
	}
	...
	for(j=0;j<mygridcs.getRowsNum();j++){
		if(mygridcs.getRowId(j)!= undefined){
			totalpar += usaFloat(mygridcs.cells(mygridcs.getRowId(j),6).getValue());
			totaliva += usaFloat(mygridcs.cells(mygridcs.getRowId(j),5).getValue());
		}
	}
