...
$status=0;
	...
	if(!empty($_REQUEST['accion']) and $_REQUEST['accion']=='Guardar'){
		$sql="INSERT INTO vehiculo.modelo (descripcion, status) VALUES ('$descripcion', $status)";
	...
	if(!empty($_REQUEST['accion']) and $_REQUEST['accion']=='Actualizar'){
		...
		$sql="UPDATE vehiculo.modelo SET descripcion='$descripcion', status=".$status." WHERE cod_mod=".$cod_mod;
		$msj="Registro Actualizado con éxito";
	}
		$conn->Execute($sql);

$sql="SELECT * FROM vehiculo.modelo ORDER BY descripcion ASC";
$rs = $conn->Execute($sql);
print $conn->ErrorMsg();
...
?>
<tr class="filas"> 
<td><?=$rs->fields['cod_mod']?></td>
<td><?=$rs->fields['descripcion']?></td>
<td align="center"><?=$text_status?></td>
<td align="center"><a href="#" onclick="gettpl(<?=$rs->fields['cod_mod']?>);"><img src="images/actualizar.gif" width="16" height="10" border="0"></a></td>
</tr>
<?php 
			$rs->MoveNext();
		}
