...
class contrato_servicio{
	...
	*****************************/
	...
	function get($conn, $id, $escEnEje=''){
		$q = "SELECT contrato_servicio.*, unidades_ejecutoras.descripcion AS unidad_ejecutora, ";
		...
		$q.= "FROM contrato_servicio ";
		$q.= "INNER JOIN unidades_ejecutoras ON (contrato_servicio.id_unidad_ejecutora = unidades_ejecutoras.id) ";
		$q.= "INNER JOIN proveedores ON (contrato_servicio.id_proveedor = proveedores.id) ";
		$q.= "WHERE contrato_servicio.id='$id'  ";
		//die($q);
		$r = $conn->Execute($q);
		if(!$r->EOF){
			$this->id = $r->fields['id'];
	...
	function get_all($conn, $escEnEje,$orden="id"){
		$q = "SELECT * FROM contrato_servicio ";
		$q.= "ORDER BY $orden ";
		$r = $conn->Execute($q);
		$collection=array();
		while(!$r->EOF){
	...
	function add($conn, 
		...
		$q = "INSERT INTO contrato_servicio ";
		$q.= "( ";
		$q.= "id_tipo_documento, ";
		...
		$r = $conn->execute($q);
		if($r){
				$nrodoc = getLastId($conn, 'id', 'contrato_servicio'); 
				if(
	...
	function set($conn, 
		...
		$q = "UPDATE contrato_servicio SET  ";
	...
	function del($conn, $id){
		$q = "DELETE FROM contrato_servicio WHERE id='$id'";
		if($conn->Execute($q)){
			$this->msj = REG_DEL_OK;
			return true;
		}else{
	...
	function addRelacionPartidas($conn, 
		...
		if(is_array($JsonRec->servicio)){
			foreach ($JsonRec->servicio as $oCS_Aux){
				$CS_Aux_2= $oCS_Aux[2];
			$q = "INSERT INTO relacion_contrato_servicio ";
			$q.= "( id_parcat, id_categoria_programatica, id_partida_presupuestaria, monto, id_contrato_servicio,porc_iva,monto_exc) ";
			$q.= "VALUES ";
	...
	function delRelacionPartidas($conn, $nrodoc){
		$q = "DELETE FROM puser.relacion_contrato_servicio WHERE id_contrato_servicio='$nrodoc'";
		if($conn->Execute($q))
			return true;
		else
			return false;
	...
	function getRelacionPartidas($conn, $id, $escEnEjec){
		$q = "SELECT relacion_contrato_servicio.*, partidas_presupuestarias.descripcion AS partida_presupuestaria,  ";
		$q.= "categorias_programaticas.descripcion AS categoria_programatica ";
		$q.= "FROM relacion_contrato_servicio  ";
		$q.= "INNER JOIN partidas_presupuestarias ON (relacion_contrato_servicio.id_partida_presupuestaria = partidas_presupuestarias.id) ";
		$q.= "INNER JOIN categorias_programaticas ON (relacion_contrato_servicio.id_categoria_programatica = categorias_programaticas.id) ";
		$q.= "WHERE relacion_contrato_servicio.id_contrato_servicio='$id' ";
	...
	function aprobar($conn, $id_contrato_servicio,
										$id_usuario,
										$id_unidad_ejecutora,
										$id_proveedor,
										$ano,
		...
		/*for($i = 0; $i < $contador; $i++){
			...
			$r = $conn->Execute($q);
			if($r){
				//die($JsonRec->servicio[$i][2]);
				if($r->fields['disponible'] < $JsonRec->servicio[$i][2]){
					$this->msj = ERROR_CS_APR_NO_DISP;
					return false;
				}
			}
		} */
		...
		if($r){
			...
			$c = $this->setNrodoc($conn, $nrodoc, $id_contrato_servicio);
			
			#ACTUALIZO LA LA FECHA DE APROBACION Y EL ESTATUS DE LA ORDEN DE PAGO#
			$d = $this->setFechaAprobacion($conn, $id_contrato_servicio);
	...
	function setNrodoc($conn, $nrodoc, $id){
		$q = "UPDATE contrato_servicio SET nrodoc = '$nrodoc' WHERE id='$id'";
		//die($q);
		if($conn->Execute($q))
			return true;
		else
	...
	function setFechaAprobacion($conn, $id){
		$q = "UPDATE contrato_servicio SET fecha_aprobacion = now(), status = '2' WHERE id='$id'";
		//die($q);
		if($conn->Execute($q))
			return true;
		else
	...
	function totalRegistroBusqueda($conn,$id_proveedor,$id_ue,$fecha_desde,$fecha_hasta,$nrodoc,$descripcion,$orden="id"){
		if(empty($id_proveedor) && empty($id_ue) && empty($fecha_desde) && empty($fecha_hasta) && empty($nrodoc) && empty($descripcion))
			return 0;
		$q = "SELECT * FROM puser.contrato_servicio ";
		$q.= "WHERE  1=1 ";
		$q.= !empty($nrodoc) ? "AND nrodoc='$nrodoc' ": "";
		$q.= !empty($fecha_desde) ? "AND fecha >='".guardafecha($fecha_desde)."' ": "";
		$q.= !empty($fecha_hasta) ? "AND fecha <='".guardafecha($fecha_hasta)."' ": "";
	...
	function anular($conn, $id, $id_usuario,
		...
		try {
		$q3 ="UPDATE contrato_servicio SET status=3 WHERE id=$id";
