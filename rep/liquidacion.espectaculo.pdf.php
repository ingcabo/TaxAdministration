<?
	...
	$id=$_REQUEST['id'];//die($_REQUEST['id']);
...
$pdf->SetFont('Courier','',8);
$pdf->Cell(100,4, $id_solicitud,0, '','C' ); 
...
$pdf->SetFont('Courier','',8);
$pdf->Cell(48,4, $tipo_espectaculo,0, '','C' );
...
$pdf->Cell(30,4, $lugar_evento." ".$lugar_evento_des,0, '','C' );

$pdf->Ln(6);
$pdf->SetFont('Courier','',8);
$pdf->Cell(28,4, 'Periodo Desde:',0, '','C' );
$pdf->Cell(20,4, muestrafecha($fec_inic),0, '','C' );
...
//$sql_entradas_1 = $conn->Execute("SELECT * FROM publicidad.entradas WHERE id_espectaculo='$id_espectaculo' LIMIT 1"); die($sql_entradas_1);
//$entradas_1 = $sql_entradas_1->fields['aforo'];//die($entradas_1);
...
$i=0;
	...
	$pdf->Cell(48,4, muestrafloat(($p->fields['cant_entradas'])*($p->fields['costo_entrada'])*($p->fields['ut_espectaculo']/100)).' Bs.',0, '','C');
	//$sub=($p->fields['cant_entradas'])*($p->fields['costo_entrada'])*($p->fields['ut_espectaculo']/100);
	//$total= $sub + $sub;
	$i++;
	$p->movenext();
				}	
/*	$pdf->Ln();
	...
	$pdf->Cell(75,4, muestrafloat($oespectaculo->total_entradas_3).' Bs.',0, '','C');*/
...
$pdf->Cell(54,4, muestrafloat($tot_impuesto),0, '','C' );
//$pdf->Cell(43.5,4, muestrafloat($oespectaculo->total_impuesto).' Bs.',0, '','R');
$pdf->Ln(4);
$pdf->Cell(175,0.1, '',1, '','C');
$pdf->Ln();
...
$banco = $sql_banco->fields['descripcion'];//die($tipo_pago);
...
if ( (!$sql_tipo_pago->EOF) && (!$sql_banco->EOF) ){	
	...
	$pdf->Cell(22,4, $oespectaculo->nro_documento,0, '','C');
	$pdf->Cell(96,4, muestrafloat($oespectaculo->total_impuesto).' Bs.',0, '','C');
...
$pdf->SetFont('Courier','B',8);
$pdf->Cell(125,4, 'Total:',0, '','R');
$pdf->Cell(40,4, muestrafloat($oespectaculo->total_impuesto).' Bs.',0, '','C');*/
