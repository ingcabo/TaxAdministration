...
$escenario = $_REQUEST['id_escenario']; 	
		...
		$rppcp = $oCp->getObjetivosSectoriales($conn, $escenario);
		//var_dump($rppcp);
...
if (is_array($rppcp)){
	foreach($rppcp as $cp){
		...
		$pdf->Cell($pdf->cellDescWidth+$pdf->cellTotalWidth, 4, utf8_decode($pdf->descPrograma), 0, '', 'C');
		$pdf->Ln();
		...
		$aux = str_replace('?',' - ',utf8_decode($cp->os));
