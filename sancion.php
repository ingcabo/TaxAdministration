...
$status=0;
	...
	if(!empty($_REQUEST['accion']) and $_REQUEST['accion']=='Guardar'){
		$sql="INSERT INTO vehiculo.sancion (descripcion, status) VALUES ('$descripcion', $status)";
	...
	if(!empty($_REQUEST['accion']) and $_REQUEST['accion']=='Actualizar'){
		...
		$sql="UPDATE vehiculo.sancion SET descripcion='$descripcion', status=".$status." WHERE cod_san=".$cod_san;
		$msj="Registro Actualizado con éxito";
	}
		
		$conn->Execute($sql);

$sql="SELECT * FROM vehiculo.sancion ORDER BY descripcion ASC";
$rs = $conn->Execute($sql);
print $conn->ErrorMsg();
...
?>
<tr class="filas"> 
<td><?=$rs->fields['cod_san']?></td>
<td><?=$rs->fields['descripcion']?></td>
<td align="center"><?=$text_status?></td>
<td align="center"><a href="#" onclick="gettpl(<?=$rs->fields['cod_san']?>);"><img src="images/actualizar.gif" width="16" height="10" border="0"></a></td>
</tr>
<?php 
			$rs->MoveNext();
		}
