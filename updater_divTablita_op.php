...
?>
	<tr>
		...
		<td>
			...
			<input type="hidden" id="categorias_programaticas_<?=$i?>" name="categorias_programaticas[]" value="<?=$relacion->id_categoria_programatica?>" />		</td>
		<td>Partida Presupuestaria:</td>
		<td>
			<input type="text" readonly value="<?=$relacion->partida_presupuestaria?>" />
			<input type="hidden" id="partidas_presupuestarias_<?=$i?>" name="partidas_presupuestarias[]" value="<?=$relacion->id_partida_presupuestaria?>" />		</td>
		<td>Monto:</td>
		<td><input value="<?=$relacion->monto?>" title="Comprometido para esta partida: <?=muestrafloat($nuevoMonto)?>" style="text-align:right" name="monto[]" id="monto_<?=$i?>" onclick="traeParCatDesdeXML($('categorias_programaticas_<?=$i?>').value, $('partidas_presupuestarias_<?=$i?>').value, this.id); suma(this.value)" class="montos" onkeypress="return(formatoNumero (this,event));" onblur="operacion(this.value, $F('momentos_presupuestarios'), $F('categorias_programaticas_<?=$i?>'), $F('partidas_presupuestarias_<?=$i?>'), this.id, this.className);" size="8" type="text" />
		...
		<input type="hidden" id="nuevoMontoParCat_<?=$i?>" name="nuevoMontoParCat[]" value="0" />		</td>
	</tr>
<? $i++; } } else{ ?>
