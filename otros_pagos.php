...
if($accion == 'Guardar'){
	$oOtrosPagos ->add($conn,$_REQUEST['banco'],$_REQUEST['nro_cuenta'],$_REQUEST['nroOtrosPagos'],$_REQUEST['id_proveedor'],guardafecha($_REQUEST['fecha']),0,$_REQUEST['contenedor_ordenes'],$usuario->id,$escEnEje,$anoCurso,$_REQUEST['observacion'],$_REQUEST['total_cheque'],$_REQUEST['descripcion'],$_REQUEST['nro_control'], $_REQUEST['beneficiario'], $_REQUEST['cuentaDestino']);
}elseif($accion == 'Actualizar'){
	$oOtrosPagos ->Anular($conn,$_REQUEST['nrodoc'],$escEnEje,$_REQUEST['status'],$_REQUEST['observacion'],$usuario->id,$anoCurso,$_REQUEST['descripcion'],$_REQUEST['nro_control']);
}
...
<span class="titulo_maestro">Emisi&oacute;n de Otros Pagos</span>
<div id="formulario">
...
<fieldset id="buscador">
	...
	<table border="0">
		<tr>
			<td width="100px">N&ordm; de Documento</td>
			<td>N&ordm; de Control</td>
		</tr>
		<tr>
			<td width="100px"><input style="width:100px" type="text" name="busca_nrodoc" id="busca_nrodoc" /></td>
			<td><input style="width:100px" type="text" name="busca_nrocontrol" id="busca_nrocontrol" /></td>
		</tr>
		<tr>
...
<?
	...
	$validator->create_message("error_nroOtrosPagos", "nroOtrosPagos", "*");
	$validator->create_message("error_benef", "beneficiario", "*");
	$validator->create_message("error_ctaDestino", "cuentaDestino", "*");
...
function traeCuentasBancarias(id_banco, div, id_cuenta,disabled){
	...
	var pars = 'combo=cuentas_bancarias&id_banco=' + id_banco + '&id_cuenta=' + id_cuenta + '&disabled=' + disabled + '&onchange=-1' +'&ms='+new Date().getTime();
...
function mostrar(){
	if ($('banco').value =='0'){
		...
		var pars = 'provee='+$('id_proveedor').value+'&ms='+new Date().getTime();
...
function selOrdenes(id,monto,montopagado,descripcion){
	Dialog.okCallback();
...
var JsonAux,PPAux=new Array;
	...
	if($("nroOtrosPagos").value==""){
		alert("Nro no puede estar vacio");
...
function busca(id_banco, id_cuenta, id_proveedor, fecha_desde, fecha_hasta, nrodoc, nrocontrol, pagina){
	var url = 'updater_busca_otros_pagos.php';
	var pars = '&id_banco=' + id_banco + '&id_cuenta=' + id_cuenta + '&id_proveedor=' + id_proveedor;
	pars += '&nrodoc=' + nrodoc + '&nrocontrol=' + nrocontrol +'&fecha_desde=' + fecha_desde+ '&fecha_hasta=' + fecha_hasta + '&pagina= ' + pagina;
	//alert(pars);
...
Event.observe('busca_nrodoc', "change", function () { 
	...
	$F('busca_fecha_hasta'),
	$F('busca_nrodoc'), 
	$F('busca_nrocontrol'),1); 
}); 

Event.observe('busca_nrocontrol', "change", function () { 
	busca($F('busca_bancos'),
	$F('busca_nro_cuenta'), 
	$F('busca_proveedores'),
	$F('busca_fecha_desde'), 
...
function sumaTotal(){
	...
	$('total_monto').value  = muestraFloat(total);;
}
function mostrar_ventana(){
	if ($('banco').value =='0'){
		...
		var pars = 'pc=1&ms='+new Date().getTime();
		var Request = new Ajax.Request(
			url,
			{
				method: 'get',
